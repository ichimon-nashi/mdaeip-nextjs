...
const ALL_HOTSPOTS = [
	...
	{ id: "clb", label: "CLB產生器" },
	{ id: "etr", label: "eTR產生器" },
	{ id: "eval-form-generator", label: "考核表產生器" },
...
export default function PatchNotes() {
	...
	const faqFeatures = [
		{ id: "ALL", label: "全部" },
		...ALL_HOTSPOTS.filter((h) =>
			faqEntries.some((e) => e.hotspot_id === h.id),
		),
	];
		...
		<div className={styles.page}>
					...
					<aside className={styles.sidebar}>
						<div className={styles.sidebarCard}>
							...
							{faqFeatures.map((f) => (
								...
								>
									<span className={styles.appFilterName}>
										{f.label}
									</span>
									{f.id !== "ALL" && (
										<span className={styles.appFilterCount}>
											{faqCounts[f.id] || 0}
										</span>
									)}
								</button>
			...
			{editorEntry !== undefined && (
				<FaqEditor
					...
					featureName={
						ALL_HOTSPOTS.find((h) => h.id === adminFilter)?.label ??
						adminFilter
					}
