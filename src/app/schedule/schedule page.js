...
export default function SchedulePage() {
		...
		const fetchSwapRequests = async () => {
			try {
				...
				data.forEach((req) => {
					...
					const aDuties = Array.isArray(req.person_a_duties)
						? req.person_a_duties
						: [];
					const bDuties = Array.isArray(req.all_duties)
						? req.all_duties
						: [];

					const aDutyByDate = {};
					aDuties.forEach((d) => {
						if (d?.date) aDutyByDate[d.date] = d.duty || "";
					});

					const bDutyByDate = {};
					bDuties.forEach((d) => {
						if (d?.date) bDutyByDate[d.date] = d.duty || "";
					});

					// Person A's cells: they RECEIVE B's duty on those dates
					const selectedDates = Array.isArray(req.selected_dates)
						? req.selected_dates
						: [];
	...
	useEffect(() => {
		if (!loading && (!user || !hasAppAccess(user, "roster"))) {
			console.log(
				"User not authenticated or no roster access, redirecting...",
			);
	...
	useEffect(() => {
		const loadScheduleData = async () => {
			...
			try {
				...
				// Get user schedule
				const userSchedule = user?.id
					? await getEmployeeSchedule(user.id, currentMonth)
					: null;
				...
				const otherSchedules = hasScheduleData
					? await getSchedulesByBase(currentMonth, activeTab).then(
							(schedules) => {
	...
	useEffect(() => {
		...
		const loadPdx = async () => {
			...
			if (monthErr || !monthRow) {
				// No published PDX — skip PDX loading but still fetch overrides below
			} else {
				...
				if (!dutiesErr && fullDuties?.length) {
			...
			// ── Fetch schedule_day_overrides — always runs, independent of PDX ──
			// schedule_day_overrides.month_id references mdaeip_schedule_months,
			// NOT pdx_months — must use its own lookup.
			try {
				const { data: schedMonthRow } = await supabase
	...
	useEffect(() => {
		const handleOutsideClick = (e) => {
			if (
				empTooltipRef.current &&
				!empTooltipRef.current.contains(e.target)
			) {
	...
	const handleNameCellClick = useCallback(
		(e, schedule) => {
			...
			if (
				empTooltipData.visible &&
				empTooltipData.employeeId === schedule.employeeID
			) {
				setEmpTooltipData((prev) => ({ ...prev, visible: false }));
				return;
			}
			...
			if (x + tooltipWidth > viewportW - 8)
				x = rect.left - tooltipWidth - 8; // flip left
	...
	const GOOGLE_CLIENT_ID =
		"238053199889-um5ssk3eprda6epciv8r5kcvia71t9oi.apps.googleusercontent.com";
	...
	const insertAllEvents = useCallback(
		async (token, calendarId, days) => {
			const events = buildEvents(days);
			if (!events.length) {
				toast("此月份無班表資料", { icon: "⚠️" });
				return;
					...
					const res = await fetch(
						`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events`,
						{
							...
							body: JSON.stringify({
								...
								start: { date: events[i].dateStr },
								end: { date: events[i].dateStr },
							}),
						},
					);
					if (res.ok) {
						successCount++;
						toast.loading(
							`新增中… (${successCount}/${events.length})`,
							{ id: toastId },
						);
					}
				} catch (err) {
	...
	const getFlightDutyDetails = useCallback(
		async (dutyCode, date) => {
			...
			try {
				const { data, error } =
					await flightDutyHelpers.getFlightDutyDetails(
						dutyCode,
						date,
						currentMonth,
					);
	...
	const handleDutyCellClick = useCallback(
		(e, employeeId, name, date, duty, sameEmployees, isUserSchedule) => {
			e.stopPropagation();
			...
			if (
				...
				tooltipData.employeeId === employeeId
			) {
				setTooltipData((prev) => ({ ...prev, visible: false }));
				return;
			}
			...
			if (pdx && baseDutyCode && /^[A-Z]/.test(baseDutyCode)) {
				...
				if (matched) {
					...
					if (matched.reporting_time)
						reportingTime = matched.reporting_time.substring(0, 5);
					if (matched.duty_end_time)
						endTime = matched.duty_end_time.substring(0, 5);
				}
			}
			...
			if (overrideMap[`${employeeId}|${date}`]) {
				...
				if (overrideEntry.start_time)
					reportingTime = overrideEntry.start_time.substring(0, 5);
				if (overrideEntry.end_time)
					endTime = overrideEntry.end_time.substring(0, 5);
			...
			if (x + tooltipWidth / 2 > viewportW - 8)
				x = viewportW - tooltipWidth / 2 - 8;
	...
	const formatDutyText = useCallback(
		(duty) => {
			if (!duty) return duty;
	...
	const getEmployeesWithSameDuty = useCallback(
		(date, duty, excludeEmployeeId = null) => {
	...
	const handleDutySelect = useCallback(
		(employeeId, name, date, duty) => {
			...
			const existingIndex = selectedDuties.findIndex(
			...
			if (existingIndex >= 0) {
				...
				setSelectedDuties((prev) => [
					...prev,
					{ employeeId, name, date, duty: displayDuty },
				]);
			}
		},
	...
	const handleDutyChangeClick = useCallback(() => {
		...
		const uniqueEmployeeIds = [
			...new Set(selectedDuties.map((duty) => duty.employeeId)),
		];
		if (uniqueEmployeeIds.length > 1) {
			toast("這位太太！一張換班單只能跟一位換班!", {
				icon: "🤨",
				duration: 3000,
			});
			return;
			...
			<thead className={styles.stickyTableHeader}>
				<tr className={styles.tableHeader}>
					{!isMobile && (
						<th
							className={
								styles.stickyCol + " " + styles.employeeId
							}
						>
							員編
						</th>
	...
	const renderTableRow = useCallback(
		(schedule, isUserSchedule = false) => {
			const isPersonA = personA?.id === schedule.employeeID;
			return (
				<tr
					key={schedule.employeeID}
					className={isPersonA ? styles.personARow : ""}
				>
					{!isMobile && (
						<td
							className={
								styles.stickyCol + " " + styles.employeeIdCell
							}
						>
							{schedule.employeeID}
						</td>
					...
					<td
						className={
							styles.stickyCol +
							" " +
							styles.employeeNameCell +
							" " +
							styles.clickableNameCell +
							(isPersonA ? " " + styles.personANameCell : "")
						}
					...
					{scheduleData.allDates.map((date) => {
						...
						const sameEmployees = getEmployeesWithSameDuty(
							date,
							duty,
						);
						const isSelected =
						...
						const swapEntry =
							swapRequestMap[`${schedule.employeeID}|${date}`];
						const swapStatus = swapEntry?.status;
						const isSwapPending = swapStatus === "pending";
						const isSwapApproved = swapStatus === "approved";

						// hasOverride: dispatch has an entry in schedule_day_overrides → red border
						const hasOverride =
							!!overrideMap[`${schedule.employeeID}|${date}`];

						const rawDisplayDuty = duty || "空";
						const formattedDisplayDuty = formatDutyText(rawDisplayDuty);
						const displayFontSizeClass = getDutyFontSize(rawDisplayDuty);
						...
						// Border priority: override (red) > approved (green) > pending (purple)
						if (hasOverride)
							...
							>
								<div
									className={
										styles.dutyContent +
										" " +
										styles[displayFontSizeClass]
									}
								>
									{formattedDisplayDuty
										.split("\n")
										.map((line, index) => (
		...
		[
			...
			overrideMap,
			formatDutyText,
			getDutyBackgroundColor,
			getDutyFontSize,
			getEmployeesWithSameDuty,
			handleDutyCellClick,
			handleNameCellClick,
			isMobile,
			personA,
			...
			{empTooltipData.visible && (
				...
				>
					<div className={styles.empTooltipHeader}>
						<span className={styles.empTooltipName}>
							{empTooltipData.name}
						</span>
						<button
					...
					<div className={styles.empTooltipBody}>
						{empTooltipData.totalFt > 0 ? (
							<>
								<div className={styles.empTooltipRow}>
									<span className={styles.empTooltipLabel}>
										總飛時
									</span>
									<span className={styles.empTooltipValue}>
										{minutesToDisplay(
											empTooltipData.totalFt,
										)}
									</span>
								</div>
								<div className={styles.empTooltipRow}>
									...
									<span className={styles.empTooltipValue}>
										{empTooltipData.amCount}早 /{" "}
										{empTooltipData.pmCount}晚
									</span>
								</div>
								<div className={styles.empTooltipRow}>
									<span className={styles.empTooltipLabel}>
								...
								<div className={styles.empTooltipRow}>
									...
									<span className={styles.empTooltipValue}>
										{empTooltipData.duties6} 班
									</span>
								</div>
							</>
						) : (
							<div className={styles.empTooltipNoData}>
								無PDX飛行資料
							</div>
						)}
						<div className={styles.empTooltipRow}>
						...
						{empTooltipData.calPicker ? (
							<div className={styles.empTooltipCalPicker}>
								...
								>
									{empTooltipData.calPicker.calendars.map(
										(cal) => (
					...
					return (
						...
						>
							...
							<div
								className={
									td.above
										? styles.schedTooltipArrowDown
										: styles.schedTooltipArrowUp
								}
							/>
							...
							<div className={styles.schedTooltipHeader}>
								<div className={styles.schedTooltipHeaderLeft}>
									<span className={styles.schedTooltipIcon}>
										<Plane size={14} />
									</span>
									<div>
										<div
											className={styles.schedTooltipDate}
										>
											{dateDisplay}
										</div>
										<div
											className={
												styles.schedTooltipDutyCode
											}
										>
											{baseDuty}
											{td.override && (
							...
							<div className={styles.schedTooltipBody}>
								...
								{(td.reportingTime || td.endTime) && (
									<div className={styles.schedTooltipRow}>
										<Clock
											size={12}
											className={
												styles.schedTooltipRowIcon
											}
										/>
										<span
											className={
												styles.schedTooltipRowLabel
											}
										>
											時間
										</span>
								...
								{pdx && sectors?.length > 0 && (
									<div
										className={
											styles.schedTooltipSectorSection
										}
									>
										<button
											className={
												styles.schedTooltipSectorToggle
											}
											onClick={() =>
												setSectorsExpanded((v) => !v)
											}
										>
											<span
												style={{ fontSize: "0.7rem" }}
										...
										{sectorsExpanded && (
											<div
												className={
													styles.schedTooltipSectorList
												}
											>
												{sectors.map((s, i) => (
													<div
														key={i}
														className={`${styles.schedTooltipSectorRow}${s.is_highlight ? " " + styles.schedTooltipSectorHighlight : ""}`}
													>
														<span
															className={
																styles.schedTooltipSectorFlight
															}
														>
															{s.flight_number}
														</span>
														<span
															className={
																styles.schedTooltipSectorRoute
															}
														>
															{s.dep_airport}
															<span
																className={
																	styles.schedTooltipSectorArrow
																}
															>
																→
															</span>
														...
														<span
															className={
																styles.schedTooltipSectorTimes
															}
														>
															{s.dep_time?.substring(
																0,
																5,
															)}{" "}
															–{" "}
															{s.arr_time?.substring(
																0,
																5,
															)}
														</span>
								...
								{(td.override?.additional_tasks || [])
									...
									.map((t, i) => {
										...
										const isBefore =
											t.start_time &&
											td.reportingTime &&
											t.start_time < td.reportingTime;
										const isAfter =
											t.end_time &&
											baseDutyEnd &&
											t.end_time > baseDutyEnd;
										const tag = isBefore
											? "前"
											: isAfter
												? "後"
										...
										return (
											<div
												key={i}
												className={
													styles.schedTooltipAdditionalTask
												}
											>
												<span
													className={
														styles.schedTooltipAdditionalTag
													}
												>
													{tag}
												</span>
												<span
													className={
														styles.schedTooltipAdditionalTitle
													}
												>
													{t.title}
												</span>
												{t.start_time && t.end_time && (
													<span
														className={
															styles.schedTooltipAdditionalTime
														}
													>
														{t.start_time}–
														{t.end_time}
								...
								{!td.isUserSchedule && (
									<button
										className={
											isAlreadySelected
												? styles.schedTooltipRemoveBtn
												: styles.schedTooltipAddBtn
										}
										onClick={() => {
											handleDutySelect(
												td.employeeId,
								...
								<div className={styles.schedTooltipCrewSection}>
									<div
										className={styles.schedTooltipCrewLabel}
									>
										同勤組員
									</div>
						...
						<div className={styles.crewScheduleSection}>
							...
							>
								<table className={styles.scheduleTable}>
									{renderTableHeader()}
									<tbody>
										{scheduleData.otherSchedules.map(
											(schedule) =>
